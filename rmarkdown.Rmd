...
df_quantile  <- quantile(df_refinau$abandono, probs= seq(0.4,1,.01),na.rm = FALSE)

#quantile(df_refinau$Prop_Abandono, probs= seq(0.9,1,.01),na.rm = TRUE)
```
```{r}
ggplot(df_quantile, aes(x = df., y = obs)) + geom_col() + scale_y_continuous(labels = scales::comma) + labs(title = "Histograma de Consumo") + xlab("Consumo (kW)") + ylab("Observaciones")
```
